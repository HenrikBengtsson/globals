...
*   checking whether the namespace can be loaded with stated dependencies ... WARNING
    ```
    Error in setRefClass("Progress", fields = list(value = "numeric", max = "numeric",  : 
      could not find function "setRefClass"
    Error: unable to load R code in package ‘uwot’
    Execution halted
...
*   checking whether package ‘SpaDES.core’ can be installed ... WARNING
    ...
    Found the following significant warnings:
      Warning: no DISPLAY variable so Tk is not available
    See ‘/home/hb/repositories/globals/revdep/checks/SpaDES.core/new/SpaDES.core.Rcheck/00install.out’ for details.
    ```
