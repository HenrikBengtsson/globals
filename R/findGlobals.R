...
find_globals_conservative <- function(expr, envir, ...) {
  objs <- character()
...
find_globals_liberal <- function(expr, envir, ...) {
  objs <- character()
...
#' @importFrom codetools makeUsageCollector walkCode
find_globals_ordered <- function(expr, envir, ...) {
...
findGlobals <- function(expr, envir = parent.frame(), ..., tweak = NULL,
  ...
  globals <- withCallingHandlers({
    ...
    find_globals_t(expr, envir = envir)
  }, warning = function(w) {
    ## Warned about '...'?
