...
print(globals_c)
stopifnot(all(globals_c %in% c("{", "<-", "c", "d", "+", "+.Date", "+.POSIXt")))
...
str(globals_c)
stopifnot(all(names(globals_c) %in% c("{", "<-", "c", "d", "+", "+.Date", "+.POSIXt")))
globals_c <- cleanup(globals_c)
str(globals_c)
stopifnot(all(names(globals_c) %in% c("c", "d")))
where <- attr(globals_c, "where")
...
str(globals_l)
stopifnot(all(names(globals_l) %in% c("{", "<-", "b", "c", "d", "+", "a", "e", "+.Date", "+.POSIXt")))
...
str(globals_i)
stopifnot(all(names(globals_i) %in% c("{", "<-", "b", "c", "d", "+", "a", "e", "+.Date", "+.POSIXt")))
...
for (method in c("ordered", "conservative", "liberal")) {
  ...
  stopifnot(all(names(globals) %in% c("{", "foo", "bar", "-", "a", "-.Date", "-.POSIXt")))
  globals <- cleanup(globals)
  str(globals)
...
str(globals)
stopifnot(all(names(globals) %in% c("{", "<-", "c", "d", "+", "+.Date", "+.POSIXt")))
...
pkgs <- packagesOf(globals)
print(pkgs)
stopifnot(length(pkgs) == 1L, pkgs == "base")
